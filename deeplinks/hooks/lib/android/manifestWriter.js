...
function injectOptions(manifestData, pluginPreferences) {
  ...
  if (!Array.isArray(launchActivity['intent-filter'])) {
    launchActivity['intent-filter'] = [];
  }
  launchActivity['intent-filter'] = launchActivity['intent-filter'].concat(ulIntentFilters);

  // Android 12+ requires android:exported when an activity has intent-filters.
  // cordova-android sets it on the main activity in newer versions, but if it's missing,
  // builds will fail when targetSdkVersion >= 31.
  if (!launchActivity['$']) {
    launchActivity['$'] = {};
  }
  if (launchActivity['$']['android:exported'] == null) {
    launchActivity['$']['android:exported'] = 'true';
  }
