...
/**
 * Get a <preference> value from config.xml by name.
 *
 * @param {String} name preference name
 * @return {String|null} preference value (as string) or null if not found
 */
ConfigXmlHelper.prototype.getPreferenceValue = function(name) {
  try {
    var configFilePath = getConfigXmlFilePath();
    var config = getCordovaConfigParser(configFilePath);
    if (typeof config.getPreference === 'function') {
      return config.getPreference(name) || null;
    }
  } catch (e) {}

  // Fallback: parse config.xml ourselves and look for <preference name="..." value="..."/>
  try {
    var xml = this.read();
    if (!xml || !xml.widget) {
      return null;
    ...
    var preferences = xml.widget.preference;
