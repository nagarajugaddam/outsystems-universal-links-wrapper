...
var pluginNpmDependencies = require(path.join(__dirname, '..', 'package.json')).dependencies;
...
function isInstallationAlreadyPerformed(ctx) {
  var pathToInstallFlag = path.join(ctx.opts.projectRoot, 'plugins', ctx.opts.plugin.id, 'deeplinks', INSTALLATION_FLAG_FILE_NAME);
  try {
...
module.exports = function(ctx) {
  ...
  var deeplinksDir = path.join(ctx.opts.projectRoot, 'plugins', ctx.opts.plugin.id, 'deeplinks');
  var result = spawnSync(npm, ['install', '--production'], { cwd: deeplinksDir });
  if (result.error) {
    throw result.error;
  }
